...
userSchema.methods.selfGenHash = function(){
...
userSchema.statics.logout = function(token,callback){        
    this.findOne({token: token},function(err,user){
        if(err){ callback('USER_OPERATION_ERROR'); return; }
        else if(!user || user=='' || user ==null){ callback('USER_NOT_FOUND',null); return;}
        else{
...
// Token Auth Every Operation
userSchema.statics.auth = function(token,callback){
    this.findOne({token: token},function(err,user){
        if(err){ callback('USER_OPERATION_ERROR'); return; }
