...
Pong = {
  ...
  Defaults: {
    ...
    ballSpeed:    <?php if(isset($_GET['ballSpeed'])){ echo $_GET['ballSpeed']; } else { echo $ballSpeed; }?>,     // should be able to cross court horizontally in 4 seconds, at starting speed ...
    ballAccel:    <?php if(isset($_GET['ballAccel'])){ echo $_GET['ballAccel']; } else { echo $ballAccel; }?>,     // ... but accelerate as time passes
    ballRadius:   <?php if(isset($_GET['ballRadius'])){ echo $_GET['ballRadius']; } else { echo $ballRadius; }?>,
    sound:        true
  },

  Colors: {
    walls:           'white',
    ball:            <?php if(isset($_GET['ballColor'])){ echo '"'.$_GET['ballColor'].'"'; } else { echo '"'.$ballColor.'"'; } ?>,
    score:           'white',
    footprint:       '#333',
  ...
  initialize: function(runner, cfg) {
    Game.loadImages(Pong.Images, function(images) {
      ...
      this.playing     = false;
      this.scores      = [0, 0];
      window.scores    = this.scores;
      this.menu        = Object.construct(Pong.Menu,   this);
      this.court       = Object.construct(Pong.Court,  this);
      this.leftPaddle  = Object.construct(Pong.LeftPaddle, this);
      this.rightPaddle = Object.construct(Pong.RightPaddle, this, true);
  ...
  start: function(numPlayers) {
    if (!this.playing) {
      ...
      this.playing = true;
      <?php
      if(isset($_GET['selectSide'])){
        if ($_GET['selectSide'] == 'Right'){
        echo 'this.rightPaddle.setAuto(numPlayers < 1, this.level(0));';
        echo 'this.leftPaddle.setAuto(numPlayers < 2, this.level(1));';
      }
        if($_GET['selectSide'] == 'Left'){
        echo 'this.leftPaddle.setAuto(numPlayers < 1, this.level(0));';
        echo 'this.rightPaddle.setAuto(numPlayers < 2, this.level(1));';
          }
    }
    else {
      echo 'this.leftPaddle.setAuto(numPlayers < 1, this.level(0));';
  ...
  onkeydown: function(keyCode) {
    switch(keyCode) {
      ...
      if(isset($_GET['selectSide'])){
            if ($_GET['selectSide'] == 'Right'){
                echo 'case Game.KEY.UP:    if (!this.rightPaddle.auto)  this.rightPaddle.moveUp();    break;';
                echo 'case Game.KEY.DOWN:    if (!this.rightPaddle.auto)  this.rightPaddle.moveDown();  break;';
                echo 'case Game.KEY.P:    if (!this.rightPaddle.auto) this.rightPaddle.moveUp();   break;';
                echo 'case Game.KEY.L:    if (!this.rightPaddle.auto) this.rightPaddle.moveDown(); break;';
            }
            if ($_GET['selectSide'] == 'Left'){
                echo 'case Game.KEY.UP:    if (!this.leftPaddle.auto)  this.leftPaddle.moveUp();    break;';
                echo 'case Game.KEY.DOWN:    if (!this.leftPaddle.auto)  this.leftPaddle.moveDown();  break;';
                echo 'case Game.KEY.P:    if (!this.rightPaddle.auto) this.rightPaddle.moveUp();   break;';
                echo 'case Game.KEY.L:    if (!this.rightPaddle.auto) this.rightPaddle.moveDown(); break;';
              }
            }
  ...
  onkeyup: function(keyCode) {
    switch(keyCode) {
      ...
      if(isset($_GET['selectSide'])){
      if  ($_GET['selectSide'] == 'Right'){
      echo 'case Game.KEY.UP: if (!this.rightPaddle.auto)  this.rightPaddle.stopMovingUp();    break;';
      echo 'case Game.KEY.DOWN: if (!this.rightPaddle.auto)  this.rightPaddle.stopMovingDown();  break;';
      echo 'case Game.KEY.P: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingUp();   break;';
      echo 'case Game.KEY.L: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingDown(); break;';
    }
    if ($_GET['selectSide'] == 'Left'){
      echo 'case Game.KEY.UP: if (!this.leftPaddle.auto)  this.leftPaddle.stopMovingUp();    break;';
      echo 'case Game.KEY.DOWN: if (!this.leftPaddle.auto)  this.leftPaddle.stopMovingDown();  break;';
      echo 'case Game.KEY.P: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingUp();   break;';
      echo 'case Game.KEY.L: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingDown(); break;';
    }
  }
  ...
  Court: {
    ...
    LeftPaddle: {
      ...
      initialize: function(pong, rhs) {
        ...
        pong.cfg.paddleHeight = <?php if(isset($_GET['leftPaddleHeight'])){ echo $_GET['leftPaddleHeight']; } else { echo $leftPaddleHeight; }?>;
      ...
      predict: function(ball, dt) {
        ...
        draw: function(ctx) {
          ctx.fillStyle = <?php if(isset($_GET['leftPaddleColor'])){ echo '"'.$_GET['leftPaddleColor'].'"'; } else { echo '"'.$leftPaddleColor.'"'; } ?>;
      ...
      RightPaddle: {
        ...
        initialize: function(pong, rhs) {
          this.pong   = pong;
          this.width  = pong.cfg.paddleWidth;
          pong.cfg.paddleHeight = <?php if(isset($_GET['rightPaddleHeight'])){ echo $_GET['rightPaddleHeight']; } else { echo $rightPaddleHeight; }?>;
          this.height = pong.cfg.paddleHeight;
          this.minY   = pong.cfg.wallWidth;
          this.maxY   = pong.height - pong.cfg.wallWidth - this.height;
          this.speed  = (this.maxY - this.minY) / pong.cfg.paddleSpeed;
        ...
        predict: function(ball, dt) {
          ...
          draw: function(ctx) {
            ctx.fillStyle = <?php if(isset($_GET['rightPaddleColor'])){ echo '"'.$_GET['rightPaddleColor'].'"'; } else { echo '"'.$rightPaddleColor.'"'; } ?>;
            ctx.fillRect(this.x, this.y, this.width, this.height);
            if (this.prediction && this.pong.cfg.predictions) {
              ctx.strokeStyle = Pong.Colors.predictionExact;
              ctx.strokeRect(this.prediction.x - this.prediction.radius, this.prediction.exactY - this.prediction.radius, this.prediction.radius*2, this.prediction.radius*2);
